// From rrobinet@github.com 03/10/2016
//
...
volatile uint8_t RFM69X::DATA[RF69_MAX_DATA_LEN];
volatile uint8_t RFM69X::_mode;        // current transceiver state
volatile uint8_t RFM69X::DATALEN;
volatile uint8_t RFM69X::SENDERID;
volatile uint8_t RFM69X::TARGETID;     // should match _address
volatile uint8_t RFM69X::PAYLOADLEN;
volatile uint8_t RFM69X::ACK_REQUESTED;
volatile uint8_t RFM69X::ACK_RECEIVED; // should be polled immediately after sending a packet with ACK request
volatile int16_t RFM69X::RSSI;         // most accurate RSSI during reception (closest to the reception)
volatile bool RFM69X::_inISR;

volatile bool RFM69X::_gotISR = false; // Added for ESP32 SPI interrupts compatibility
SPISettings settingsA(SPI_CLOCK_DIV4, MSBFIRST, SPI_MODE0); // Adapted according current SPI library (SPI_HAS_TRANSACTION)

RFM69X* RFM69X::selfPointer;

bool RFM69X::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  ...
  SPI.begin(18,19,23,5); //void SPIClass::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss)
  unsigned long start = millis();
  uint8_t timeout = 50;
  do writeReg(REG_SYNCVALUE1, 0xAA); while (readReg(REG_SYNCVALUE1) != 0xaa && millis()-start < timeout);
  // Test for functional RFM transciever
  if (millis()-start >= timeout) return false;              // return false if RFM transciever is NOT installed or doesn't function
  start = millis();
  ...
  if (millis()-start >= timeout)
    return false;
  _inISR = false;
  attachInterrupt(_interruptNum, RFM69X::isr0, RISING);
...
uint32_t RFM69X::getFrequency()
{
  return RF69_FSTEP * (((uint32_t) readReg(REG_FRFMSB) << 16) + ((uint16_t) readReg(REG_FRFMID) << 8) + readReg(REG_FRFLSB));
}

// set the frequency (in Hz)
void RFM69X::setFrequency(uint32_t freqHz)
{
...
void RFM69X::setMode(uint8_t newMode)
{
  if (newMode == _mode)
    return;
...
void RFM69X::setAddress(uint8_t addr)
{
  _address = addr;
...
void RFM69X::setNetwork(uint8_t networkID)
{
...
void RFM69X::setPowerLevel(uint8_t powerLevel)
{
...
void RFM69X::send(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK)
{
...
bool RFM69X::sendWithRetry(uint8_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries, uint8_t retryWaitTime) {
  uint32_t sentTime;
...
bool RFM69X::ACKReceived(uint8_t fromNodeID) {
  if (receiveDone())
...
bool RFM69X::ACKRequested() {
  return ACK_REQUESTED && (TARGETID != RF69_BROADCAST_ADDR);
}

// should be called immediately after reception in case sender wants ACK
void RFM69X::sendACK(const void* buffer, uint8_t bufferSize) {
...
// internal function
void RFM69X::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
...
// interruptHandler has beeing strip down for ESP32 extended compatibility (SPI transfer now moved to interruptHandling())
void RFM69X::interruptHandler() {
  if (_mode == RF69_MODE_RX) // Ensure interrupts occurs when RFM69 is in receive mode
  {
//  	 Serial.println ("Interrupt Handler activated");
    _gotISR = true; 	// Indicate a possible data correct data message is recieved (check done in interruptHandling function
  }
  else _gotISR = false;
}

// This function replaces the inetrruptHandler SPI transcation commands that are not allowed in interrupt routine for ESP32 
bool RFM69X::interruptHandling() {	

  if (_gotISR && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY)) // Ensure data is receieved with a correct payload
  {
 //    Serial.println ("Interrupt Handling validated");
  	RSSI = readRSSI();
  	setMode(RF69_MODE_STANDBY); // ensure mode is not RX while collecting the SPI data 
    select();
    SPI.transfer(REG_FIFO & 0x7F); // Set SPI register pointer to FIFO
    PAYLOADLEN = SPI.transfer(0);  // Get the Payload length
    PAYLOADLEN = PAYLOADLEN > 66 ? 66 : PAYLOADLEN; // precaution
    TARGETID = SPI.transfer(0);    // Get the Target ID
    ...
    SENDERID = SPI.transfer(0);    // Get the Sender ID
    uint8_t CTLbyte = SPI.transfer(0); // Get the Control byte
    DATALEN = PAYLOADLEN - 3;		   // Actual data lenght (payload - header)
    // Serial.print ("PAYLOADLEN = ");Serial.println (PAYLOADLEN);
    interruptHook(CTLbyte);     // TWS: hook to derived class interrupt function  
    for (uint8_t i = 0; i < DATALEN; i++) // Get the payload data
    {
...
// internal function
void RFM69X::isr0() { _inISR = true; selfPointer->interruptHandler(); _inISR = false; }

// internal function
void RFM69X::receiveBegin() {
  DATALEN = 0;
  SENDERID = 0;
  TARGETID = 0;
  PAYLOADLEN = 0;
...
bool RFM69X::receiveDone() {
//ATOMIC_BLOCK(ATOMIC_FORCEON)
//{
  interruptHandling(); // Comptue SPI data if RFM69 SPI interrupts occurs (used for ESP32 compatibility)
...
void RFM69X::encrypt(const char* key) {
  setMode(RF69_MODE_STANDBY);
  if (key != 0)
  {
    select();
...
int16_t RFM69X::readRSSI(bool forceTrigger) {
  int16_t rssi = 0;
  if (forceTrigger)
  {
...
uint8_t RFM69X::readReg(uint8_t addr)
{
  select();
...
void RFM69X::writeReg(uint8_t addr, uint8_t value)
{
  select();
...
void RFM69X::promiscuous(bool onOff) {
  _promiscuousMode = onOff;
  //writeReg(REG_PACKETCONFIG1, (readReg(REG_PACKETCONFIG1) & 0xF9) | (onOff ? RF_PACKET1_ADRSFILTERING_OFF : RF_PACKET1_ADRSFILTERING_NODEBROADCAST));
}

// for RFM69HW only: you must call setHighPower(true) after initialize() or else transmission won't work
void RFM69X::setHighPower(bool onOff) {
  _isRFM69HW = onOff;
...
// internal function
void RFM69X::setHighPowerRegs(bool onOff) {
  writeReg(REG_TESTPA1, onOff ? 0x5D : 0x55);
  writeReg(REG_TESTPA2, onOff ? 0x7C : 0x70);
}

// set the slave select (CS) pin 
void RFM69X::setCS(uint8_t newSPISlaveSelect) {
...
void RFM69X::readAllRegsCompact() {
...
uint8_t RFM69X::readTemperature(uint8_t calFactor) // returns centigrade
{
  setMode(RF69_MODE_STANDBY);
...
void RFM69X::rcCalibration()
{
  writeReg(REG_OSC1, RF_OSC1_RCCAL_START);
  while ((readReg(REG_OSC1) & RF_OSC1_RCCAL_DONE) == 0x00);
}

inline void RFM69X::maybeInterrupts()
{
